...
BACKTICKS = "```" # Mencegah error rendering markdown
...
SYSTEM_PROMPT = f"""Anda adalah Lagos AI 9.1 (Rian Dev), asisten analitik tingkat tinggi dan Web Developer.
...
{BACKTICKS}
Berikan kode yang bersih, modern, dan bebas bug."""
...
class MediaUtils:
    ...
    def ekstrak_kode_html(teks: str) -> Optional[str]:
        # Menggunakan BACKTICKS dari konstanta agar rendering markdown tidak rusak
        pola_regex = rf'{BACKTICKS}html\n(.*?)\n{BACKTICKS}'
        match = re.search(pola_regex, teks, re.DOTALL | re.IGNORECASE)
...
def inject_custom_css():
    st.markdown("""
        <style>
            @import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@300;400;600;700&family=Fira+Code:wght@400;500&display=swap');
            html, body, [class*="css"] { font-family: 'Plus Jakarta Sans', sans-serif; }
            #MainMenu {visibility: hidden;}
            footer {visibility: hidden;}
            
            /* CSS Khusus untuk Code Editor agar terlihat seperti Replit */
            .stTextArea textarea {
...
def render_login_ui():
    st.markdown('<div class="header-title">🔮 Lagos Workspace</div>', unsafe_allow_html=True)
    st.markdown('<div class="header-subtitle">Silakan Masuk untuk Mengakses Lingkungan Pengembangan</div>', unsafe_allow_html=True)
...
def init_session_state():
    defaults = {
        ...
        "temp_doc": None,
        "uploader_key": 0,
        "current_code": "<!-- Web App Preview Ready -->\n<h1>Halo dari Lagos Workspace</h1>\n<p>Minta AI untuk membuat web app, lalu edit kodenya di sini!</p>"
    }
...
def main():
    st.set_page_config(
        page_title="Lagos Workspace IDE",
        page_icon="💻",
        layout="wide", 
    ...
    # --- SIDEBAR: MANAJEMEN FILE & HISTORY (Khas Replit) ---
    with st.sidebar:
        st.success(f"👤 Workspace: **{st.session_state.username}**")
        
        if st.button("➕ Proyek/Obrolan Baru", use_container_width=True, type="primary"):
            st.session_state.current_session_id = None
            st.session_state.messages = [{"role": "system", "content": SYSTEM_PROMPT}]
            st.session_state.current_code = "<!-- Proyek Baru -->"
            st.rerun()

        st.markdown("### 🗂️ Riwayat Proyek")
                ...
                for sess_id, title in sessions:
                    ...
                    with col_btn:
                        ...
                        if st.button(title, key=f"btn_{sess_id}", use_container_width=True, type=btn_type):
                            ...
                            # Saat memuat history, cari kode HTML terakhir untuk dimasukkan ke editor
                            last_code = ""
                            for msg in reversed(st.session_state.messages):
                                if msg["role"] == "assistant":
                                    extracted = MediaUtils.ekstrak_kode_html(str(msg["content"]))
                                    if extracted:
                                        last_code = extracted
                                        break
                            if last_code:
                                st.session_state.current_code = last_code
                                
                            st.rerun()
                    with col_del:
                        if st.button("🗑️", key=f"del_{sess_id}", help="Hapus"):
        ...
        st.divider()
        st.markdown("### ⚙️ Konfigurasi Environment")
        ...
        if len(st.session_state.messages) > 1:
            ...
            st.download_button(
                ...
                data=file_word,
                file_name="Lagos_Workspace_Report.docx",
    ...
    # --- MAIN AREA: IDE LAYOUT (Tabbed Workspace) ---
    tab_chat, tab_editor, tab_preview = st.tabs(["💬 Asisten AI", "💻 Code Editor", "🌐 Live Preview"])
        ...
        with chat_container:
            for idx, message in enumerate(st.session_state.messages):
                ...
                with st.chat_message(message["role"]):
                    ...
                    # Menampilkan Markdown, tapi tidak merender pop-up.
                    st.markdown(text_disp)
                    
                    if message["role"] == "assistant":
                        html_code = MediaUtils.ekstrak_kode_html(text_disp)
                        if html_code:
                            st.info("💡 Kode telah berhasil disuntikkan ke Editor! Buka tab 'Code Editor' atau 'Live Preview' di atas.")

        # Input Area (berada di dalam tab Chat)
        with st.container():
            ...
            with col_input:
                prompt_text = st.chat_input("Tanyakan sesuatu ke Lagos AI...")
    ...
    # Logika untuk merespons AI (Dijalankan di luar blok UI input agar aman saat Rerun)
        ...
        with tab_chat:
            with st.chat_message("assistant"):
                ...
                try:
                    ...
                    # Cek apakah AI memproduksi HTML, jika ya, perbarui Editor
                    new_code = MediaUtils.ekstrak_kode_html(full_response)
                    if new_code:
                    ...
                    st.error(f"Kesalahan engine: {str(e)}")
                    st.session_state.messages.pop()

    # === TAB 2: CODE EDITOR ===
    with tab_editor:
        st.markdown("🔧 **Live Code Editor (HTML/CSS/JS)**")
        # Text Area berfungsi sebagai editor interaktif, layaknya panel tengah Replit
        updated_code = st.text_area(
            "Tulis atau edit kode Anda di sini:", 
            value=st.session_state.current_code, 
            height=500,
            label_visibility="collapsed"
        )
        if updated_code != st.session_state.current_code:
            st.session_state.current_code = updated_code
            
    # === TAB 3: LIVE PREVIEW ===
    with tab_preview:
        st.markdown("🚀 **Hasil Render Aplikasi**")
        injection = """<base target="_blank"><script>document.addEventListener("click", function(e) {var target = e.target.closest("a");if(target && target.href) {target.setAttribute("target", "_blank");}});</script>"""
        html_render = injection + "\n" + st.session_state.current_code
        
        with st.container(border=True):
            components.html(html_render, height=550, scrolling=True)
